...
def main():
    ...
    dev = Agilent33220A(args.device)
    logging.info("Established connection.")

    try:
        logging.info("Starting server at port {args.port}... ")
        simple_server_loop({"Agilent33220A": dev}, sca.bind_address_from_args(args),args.port)
    finally:
        dev.close()
