...
def get_argparser():
    ...
    parser.add_argument(
        "-s", "--dlc-server", required=True, help="Laser controller address / hostname."
    )
    parser.add_argument(
        "--firmware", default="v3_0_1", help="Firmware version of DLC Pro"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--database", default="dl_pro_status", help="Influx database name."
    )
    parser.add_argument(
        "--poll", default=30, type=int, help="Measurement polling period (seconds)"
    )
    parser.add_argument(
        "--config-file", help="Path to json file configuring parameters to monitor."
    )
    parser.add_argument(
...
def main():
    ...
    async def stream_data():
        ...
        while True:
            async with dlcpro_sdk.Client(
                dlcpro_sdk.NetworkConnection(args.dlc_server)
            ) as dlc:
                ...
                while True:
                    ...
                    for key in parameters.keys():
                        try:
                            msg[key] = await wait_for(
                                dlc.get(parameters[key], float), timeout=args.timeout
                            )
