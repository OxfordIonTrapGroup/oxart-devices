...
def get_argparser():
    parser = argparse.ArgumentParser(description="ARTIQ controller for SCPI device.")
...
def main():
    ...
    dev = SCPIDevice(addr=args.ipaddr, serial_number=args.serialnumber)

    try:
        simple_server_loop({"scpi_device": dev}, args.bind, args.port)
    finally:
        dev.close()
