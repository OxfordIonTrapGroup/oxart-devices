...
class ScpiDmm:
    ...
    def measure(self):
        ...
        The device must already be configured. Note for slow measurements (low BW, high
        integration time, auto-zero enabled) over GPIB may hang due to the "read_eoi"
        command hitting a timeout. To avoid this, use :meth initiate_measurement:
        followed by :meth fetch_result:.
        """
        self.stream.write("READ?\n".encode())
        return float(self.stream.readline().decode())

    def initiate_measurement(self):
        """Triggers a measurement, without transferring the results to the device's
        output buffer.

        The device must already be configured for the measurement.
        """
        self.stream.write("INIT\n".encode())

    def fetch_result(self):
        """Fetch measurement results from the device's output buffer (see :meth
    ...
    def set_measurement_mode(self, mode):
        ...
        :param mode: a measurement mode string. Typically one of: "volt:dc";
            ...
            "fresistance" (4-wire resistance); "frequency"; "period"; "continuity";
            "diode"
        """
    ...
    def set_range(self, measurement_range):
        ...
        :param measurement_range: the measurement range. Either a number or one of
            "min"; "max"
        """
    ...
    def set_resolution(self, resolution):
        ...
        :param resolution: the measurement resolution. Either a number or one of "min";
            "max" (max is the worst resolution)
        """
        mode = self.get_measurement_mode()
    ...
    def set_integration_time(self, t_int):
        """Sets the measurement integration time :param t_int: the measurement
        integration time in units of mains power cycles (NPLC).

        One of: 0.02, 0.2, 1, 10, 100, "min", "max"
        """
    ...
    def set_bw(self, bandwidth):
        """Sets the measurement bandwidth :param bandwidth: the measurement bandwidth
        (Hz), one of: 3; 20; 200; "min"; "max"."""
    ...
    def set_auto_impedance(self, enabled):
        ...
        Only affects DC voltage measurements. With auto impedance off, the input
        impedance is 10M for all ranges. With it on, the impedance is >10G for the 0.1V,
        1V and 10V ranges.
        """
        if enabled:
    ...
    def get_range(self):
        """Returns the current measurement range as a float NB this disables auto-
        range."""
        mode = self.get_measurement_mode()
