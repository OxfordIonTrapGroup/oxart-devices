...
class IIR:
    ...
    def configure_biquad(self, zeros, poles, gain=1.0):
        ...
        num_coefs = z_transform(
            get_polynomial_coefs([2 * np.pi * x for x in zeros]), self.t_update
        )
        denom_coefs = z_transform(
            get_polynomial_coefs([2 * np.pi * x for x in poles]), self.t_update
        )
...
class GPIO_HDR_SPI:
    ...
    def set_gpio_hdr(self, gpio_hdr_word):
        assert (
            gpio_hdr_word >= 0 and gpio_hdr_word <= 0xFFFF
