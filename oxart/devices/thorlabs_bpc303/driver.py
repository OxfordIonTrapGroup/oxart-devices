...
class _APTCardSlotDevice:
    ...
    def ack_status_update(self):
        self._send_message(
            Message(MGMSG.PZ_ACK_PZSTATUSUPDATE, dest=SRC_DEST["RACK_CONTROLLER"].value)
        )
...
class BPC303(_APTCardSlotDevice):
    ...
    def __init__(self, port, enable_feedback=False):
        super().__init__(port)
        logger.info(
            "Device vlimit is {}, travel is {}um".format(PZ_MAX_VOLTAGE, PZ_TRAVEL_UM)
        )
    ...
    def set_voltage(self, bay_id, voltage, channel=0):
        ...
        self._send_message(
            Message(MGMSG.PZ_SET_OUTPUTVOLTS, dest=self.bays[bay_id - 1], data=payload)
        )
    ...
    def set_position(self, bay_id, position, channel=0):
        ...
        self._send_message(
            Message(MGMSG.PZ_SET_OUTPUTPOS, dest=self.bays[bay_id - 1], data=payload)
        )
    ...
    def set_voltage_limit(self, bay_id, voltage, channel=0):
        if voltage > 150 or voltage < 0:
            raise ValueError("{}V not between 0V and 150V".format(voltage))
        payload = struct.pack("<HHH", channel, int(10 * voltage), 0)
        self._send_message(
            Message(
                MGMSG.PZ_SET_OUTPUTMAXVOLTS, dest=self.bays[bay_id - 1], data=payload
            )
        )
    ...
    def _check_position_in_limit(self, position):
        ...
        if position > PZ_TRAVEL_UM or position < 0:
            raise ValueError(
                "Position {}μm not between 0μm and travel={}μm".format(
