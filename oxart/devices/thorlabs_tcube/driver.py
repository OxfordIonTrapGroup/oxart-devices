...
class _Tcube:
    ...
    async def module_identify(self):
        ...
        Instruct hardware unit to identify itself by flashing its front panel led.
        """
    ...
    async def hardware_start_update_messages(self, update_rate):
        ...
        Status update messages contain information about the position and status of the
        controller.
...
class Tpz(_Tcube):
    """Either :py:meth:`set_tpz_io_settings()<Tpz.set_tpz_io_settings>` or
    :py:meth:`get_tpz_io_settings()<Tpz.get_tpz_io_settings>` must be completed to
    finish initialising the driver."""
    ...
    async def set_position_control_mode(self, control_mode):
        ...
        When in closed-loop mode, position is maintained by a feedback signal from the
        piezo actuator. This is only possible when using actuators equipped with
        position sensing.
        :param control_mode: 0x01 for Open Loop (no feedback). 0x02 for Closed Loop
    ...
    async def get_position_control_mode(self):
        ...
        :return: Returns the control mode. 0x01 for Open Loop (no feedback). 0x02 for
            Closed Loop (feedback employed). 0x03 for Open Loop Smooth. 0x04 for Closed
            Loop Smooth.
        :rtype: int
        """
    ...
    async def set_output_position(self, position_sw):
        ...
        This command is only applicable in Closed Loop mode. If called when in Open Loop
        mode, it is ignored. The position of the actuator is relative to the datum set
        for the arrangement using the ZeroPosition method.
        :param position_sw: The output position of the piezo relative to the zero
            position. The voltage is set in the range [0; 32767] or [0; 65535] depending
            on the unit. This corresponds to 0 to 100% of the maximum piezo extension.
        """
    ...
    async def set_input_volts_source(self, volt_src):
        """Set the input source(s) which controls the output from the HV amplifier
        circuit (i.e. the drive to the piezo actuators).
    ...
    async def get_input_volts_source(self):
        """Get the input source(s) which controls the output from the HV amplifier
        circuit.
    ...
    async def set_pi_constants(self, prop_const, int_const):
        ...
        These parameters determine the response characteristics when operating in closed
        loop mode. The processors within the controller compare the required (demanded)
        position with the actual position to create an error, which is then passed
        through a digital PI-type filter. The filtered value is used to develop an
        output voltage to drive the pizeo.
    ...
    async def get_pi_constants(self):
        ...
        :return: Returns a tuple whose first element is the proportional term and the
    ...
    async def get_output_lut(self):
        ...
        :return: a tuple whose first element is the lut index and the second is the
    ...
    async def set_output_lut_parameters(self, mode, cycle_length, num_cycles,
        ...
        the user to define a voltage or position sequence to be output, either
        periodically or a fixed number of times, with a selectable interval between
        adjacent samples. This waveform generation function is particularly useful
        for operations such as scanning over a particular area, or in any other
        application that requires a predefined movement sequence. This function is
        used to set parameters which control the output of the LUT array.
    ...
    async def set_eeprom_parameters(self, msg_id):
        ...
        :param msg_id: The message ID of the message containing the parameters to be
            saved.
        """
    ...
    async def set_tpz_io_settings(self, voltage_limit, hub_analog_input):
        """Set various I/O settings." :param voltage_limit: The piezo actuator
        connected to the T-Cube has a specific maximum operating voltage.
    ...
    async def get_tpz_io_settings(self):
        ...
        :return: Returns a tuple whose elements are the voltage limit and the Hub analog
            input. Refer to :py:meth:`set_tpz_io_settings() <Tpz.set_tpz_io_settings>`
...
class Tdc(_Tcube):
    ...
    async def set_pot_parameters(self, zero_wnd, vel1, wnd1, vel2, wnd2, vel3, wnd3,
        ...
        :param wnd1: The deflection from the mid position (in ADC counts zero_wnd to
            127) to apply vel1.
        :param vel2: The velocity to move when between wnd1 and wnd2.
        :param wnd2: The deflection from the mid position (in ADC counts wnd1 to 127) to
            apply vel2.
        :param vel3: The velocity to move when between wnd2 and wnd3.
        :param wnd3: The deflection from the mid position (in ADC counts wnd2 to 127) to
            apply vel3.
    ...
    async def set_position_counter(self, position):
        ...
        In general, this command is not normally used. Instead, the stage is homed
        immediately after power-up; and after the homing process is completed, the
        position counter is automatically updated to show the actual position.
    ...
    async def set_encoder_counter(self, encoder_count):
        ...
        This is only applicable to stages and actuators fitted with an encoder. In
        general this command is not normally used. Instead the device is homed at power-
        up.
    ...
    async def set_jog_parameters(self, mode, step_size, acceleration, max_velocity,
        ...
        :param acceleration: The acceleration in encoder counts/sec/sec.
        :param max_velocity: The maximum (final) velocity in encoder counts/sec.
    ...
    async def get_jog_parameters(self):
        ...
        :return: A 5 int tuple containing in this order: jog_mode, step_size,
    ...
    async def set_gen_move_parameters(self, backlash_distance):
        ...
        :param backlash_distance: The value of the backlash distance, which specifies
            the relative distance in position counts.
        """
    ...
    async def set_limit_switch_parameters(self, cw_hw_limit, ccw_hw_limit):
        ...
        clockwise hardware limit switch when contact is made. 0x01 Ignore switch or
        switch not present. 0x02 Switch makes on contact.
    ...
    async def get_limit_switch_parameters(self):
        ...
        :return: A 2 int tuple containing the following in order: cw_hw_limit,
            ccw_hw_limit. Cf. description in :py:meth:`set_limit_switch_parameters()
    ...
    async def move_relative_memory(self):
        """Start a relative move of distance in the controller's memory The relative
    ...
    async def move_absolute_memory(self):
        ...
        The absolute move position parameter used for the move will be the parameter
    ...
    async def move_absolute(self, absolute_distance):
        ...
        :param absolute_distance: The distance to move. This is a signed integer that
    ...
    async def move_velocity(self, direction):
        ...
        When this method is called, the motor will move continuously in the specified
        direction using the velocity parameter set by the
        :py:meth:`set_move_relative_parameters() <Tdc.set_move_relative_parameters>`
    ...
    async def set_dc_pid_parameters(self,
        ...
        :param integral_limit: The integral limit parameter is used to cap the value of
            the integrator to prevent runaway of the integral sum at the output. Values
            are in range [0; 32767]. If set to 0, then integration term in the PID loop
            is ignored.
        :param filter_control: Identifies which of the above are applied by setting the
    ...
    async def get_dc_pid_parameters(self):
        ...
        :return: A 5 int tuple containing in this order: proportional gain, integral
            gain, differential gain, integral limit and filter control. Cf.
            :py:meth:`set_dc_pid_parameters() <Tdc.set_dc_pid_parameters>` for precise
            description.
        :rtype: A 5 int tuple.
        """
    ...
    async def set_av_modes(self, mode_bits):
        ...
        The LED on the control keyboard can be configured to indicate certain driver
        states.
        :param mode_bits: Set the bit 0 will make the LED flash when the 'Ident' message
            is sent. Set the bit 1 will make the LED flash when the motor reaches a
            forward or reverse limit switch. Set the bit 3 (value 8) will make the LED
            lit when motor is moving.
        """
    ...
    async def set_button_parameters(self, mode, position1, position2):
        ...
        The control keypad can be used either to jog the motor, or to perform moves to
        absolute positions. This function is used to set the front panel button
        functionality.
        :param mode: If set to 1, the buttons are used to jog the motor. Once set to
            this mode, the move parameters for the buttons are taken from the arguments
            of the :py:meth:`set_jog_parameters() <Tdc.set_jog_parameters>` method. If
            set to 2, each button can be programmed with a differente position value
            such that the controller will move the motor to that position when the
            specific button is pressed.
        :param position1: The position (in encoder counts) to which the motor will move
            when the top button is pressed.
        :param position2: The position (in encoder counts) to which the motor will move
    ...
    async def get_button_parameters(self):
        ...
        :return: A 3 int tuple containing in this order: button mode, position1 and
            position2. Cf. :py:meth:`set_button_parameters()
    ...
    async def get_dc_status_update(self):
        ...
        :return: A 3 int tuple containing in this order: position, velocity, status
            bits.
        :rtype: A 3 int tuple
        """
    ...
    async def suspend_end_of_move_messages(self):
        """Disable all unsolicited "end of move" messages and error messages returned
        by the controller.
    ...
    async def resume_end_of_move_messages(self):
        """Resume all unsolicited "end of move" messages and error messages returned
        by the controller.
...
class Kpc(Tpz):
    ...
    Inherits from Tpz but overrides methods that are specific to TPZ devices and not
    supported by KPC devices.
    """

    async def set_tpz_io_settings(self, voltage_limit, hub_analog_input):
        raise NotImplementedError("set_tpz_io_settings not supported on KPC devices")

    async def get_tpz_io_settings(self):
        raise NotImplementedError("get_tpz_io_settings not supported on KPC devices")
...
class KpcSim(TpzSim):
    """KP101 K-Cube Piezo Controller driver simulation.

    Inherits from TpzSim but overrides methods that are specific to KPC devices and not
    supported by Tpz devices.
    """
