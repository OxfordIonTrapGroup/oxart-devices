...
class HolzworthSynthRaw:
    ...
    def __init__(self):
        ...
        self.exponent_dict = {
            v: k for k, v in self.suffix_dict.items()
    ...
    def set_freq(self, freq):
        ...
        exponent = int(
            3.0 * np.floor(np.log10(freq) / 3.0)
        )  # find nearest SI suffix exponent (i.e. 0,3,6 or 9)

        try:
            # rounding to 3 d.p. as otherwise synth can set to the wrong frequency.
            freq_string = (
                str(round(freq / (10**exponent), exponent + 3))
