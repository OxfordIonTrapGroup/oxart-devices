...
class GPIB:
    """Simple pySerial-compatible wrapper for GPIB devices connected to a Prologix
    Ethernet/USB <-> GPIB  adapter.

    NB this implementation does not auto-escape data sent to the GPIB controller. As a
    result, any lines beginning with "++" are interpreted as commands for the GPIB
    controller itself instead of being passed on to a GPIB device.
    """
    ...
    def read(self, size=1, gpib_addr=None):
        ...
        If a timeout is set it may return fewer characters than requested, otherwise it
        blocks until the requested number of bytes have been read.
        """
        self.set_addr(gpib_addr)
    ...
    def readline(self, gpib_addr=None):
        """Return a line terminated with a single '\n' character from the GPIB device
        at gpib_addr.

        If gpib_addr is None, we write to the last address set. The final '\n' character
        and any immediately preceding '\r's are trimmed.

        If a timeout has been set for this stream and no line is read within the timeout
    ...
    class Stream:
        ...
        def read(self, size=1):
            ...
            If a timeout is set it may return fewer characters than requested, otherwise
            it blocks until the requested number of bytes have been read.
            """
            return self.bus.read(size, self.addr)

        def readline(self, gpib_addr=None):
            """Returns a line terminated with a single '\n' character from the GPIB
            device .

            The final '\n' character and any immediately preceding '\r's are trimmed.
            """
