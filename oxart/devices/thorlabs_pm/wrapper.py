"""
Python wrapper for Thorlabs power meter device driver.

Most of the code in this module has been taken directly from the example file
provided with the Thorlabs Optical Power Monitor software. The docstrings
therefore do not conform to PEP 8.
"""
...
# driver directory. Any changes in that file would need to be reflected here
TLPM_ATTR_SET_VAL = 0
TLPM_ATTR_MIN_VAL = 1
TLPM_ATTR_MAX_VAL = 2
...
class TLPM:
    ...
    def open(self, resourceName, IDQuery, resetDevice):
        ...
        (1) Opens a session to the Default Resource Manager resource and a
            session to the specified device using the Resource Name.
        ...
        Args:
            ...
            IDQuery (c_bool):This parameter specifies whether an
                identification query is performed during the initialization
                process.
                VI_OFF (0): Skip query.
                VI_ON  (1): Do query (default).

            resetDevice (c_bool): This parameter specifies whether the
                instrument is reset during the initialization process.
                VI_OFF (0) - no reset
        ...
        self.devSession.value = 0
        pInvokeResult = self.dll.TLPM_init(resourceName, IDQuery, resetDevice,
    ...
    def findRsrc(self, resourceCount):
        """
        This function finds all driver compatible devices attached to the PC
        and returns the number of found devices.

        Note: The function additionally stores information like system name
              about the found resources internally. This information can be
              retrieved with further functions from the class,
              e.g. <Get Resource Description> and <Get Resource Information>.


        Args:
            resourceCount(c_int use with byref): The number of connected
                devices that are supported by this driver.
    ...
    def getRsrcName(self, index, resourceName):
        ...
        Args:
            ...
            resourceName(create_string_buffer): This parameter returns the
                resource descriptor. Use this descriptor to specify the device
                in <Initialize>.
                Note: The array must contain at least TLPM_BUFFER_SIZE (256)
                      elements ViChar[256].

        Returns:
            int: The return value, 0 is for success
        """
        pInvokeResult = self.dll.TLPM_getRsrcName(self.devSession, index,
                                                  resourceName)
        self.__testForError(pInvokeResult)
        return pInvokeResult

    def getRsrcInfo(self, index, modelName, serialNumber, manufacturer,
                    deviceAvailable):
        """
        ...
        Args:
            ...
            modelName(create_string_buffer): This parameter returns the model
                name of the device. The array must contain at least
                TLPM_BUFFER_SIZE (256) elements ViChar[256].
                You may pass VI_NULL if you do not need this parameter.
            ...
            serialNumber(create_string_buffer): This parameter returns the
                serial number of the device. The array must contain at least
                TLPM_BUFFER_SIZE (256) elements ViChar[256].
                You may pass VI_NULL if you do not need this parameter.
            ...
            manufacturer(create_string_buffer): This parameter returns the
                manufacturer name of the device. The array must contain at
                least TLPM_BUFFER_SIZE (256) elements ViChar[256].
                You may pass VI_NULL if you do not need this parameter.

                The manufacturer name is not available for serial interfaces
                over Bluetooth.

            deviceAvailable(c_int16 use with byref): Returns the information
                if the device is available. Devices that are not available are
        ...
        pInvokeResult = self.dll.TLPM_getRsrcInfo(
            self.devSession, index, modelName, serialNumber, manufacturer,
            deviceAvailable)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def readRegister(self, reg, value):
        """
        This function reads the content of any readable instrument register.
        Refer to your instrument's user's manual for more details on status
        structure registers.

        Args:
            reg(c_int16): Specifies the register to be used for operation.
                This parameter can be any of the following constants:
            ...
            value(c_int16 use with byref): This parameter returns the value of
    ...
    def setTime(self, year, month, day, hour, minute, second):
        """
        This function sets the system date and time of the power meter.
    ...
    def getTime(self, year, month, day, hour, minute, second):
        """
        This function returns the system date and time of the power meter.
    ...
    def setLineFrequency(self, lineFrequency):
        ...
        Args:
            lineFrequency(c_int16): This parameter specifies the line
        ...
        Returns:
            int: The return value, 0 is for success
        """
        pInvokeResult = self.dll.TLPM_setLineFrequency(self.devSession,
                                                       lineFrequency)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def getLineFrequency(self, lineFrequency):
        ...
        Args:
            lineFrequency(c_int16 use with byref): This parameter returns the
                selected line frequency in Hz.
        Returns:
            int: The return value, 0 is for success
        """
        pInvokeResult = self.dll.TLPM_getLineFrequency(self.devSession,
                                                       lineFrequency)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def getBatteryVoltage(self, voltage):
        """
        This function is used to obtain the battery voltage readings from the
        instrument.
    ...
    def setAvgTime(self, avgTime):
        """
        This function sets the averaging time for measurement value
        generation.

        Args:
            avgTime(c_double): This parameter specifies the average time in
                seconds. The value will be rounded to the closest multiple of
                the device's internal sampling rate.

                To get a measurement value from the device, the timeout in
                your application has to be longer than the averaging time.
    ...
    def setAttenuation(self, attenuation):
        """
        This function sets the input attenuation factor.

        Note: This function is only available on PM100A, PM100D, PM100USB,
              PM200, PM400.


        Args:
            attenuation(c_double): This parameter specifies the input
    ...
    def setWavelength(self, wavelength):
        """
        This function sets the user wavelength in nanometers (nm).

        Wavelength set value is used for calculating power.

        Args:
            wavelength(c_double): This parameter specifies the user
        ...
        pInvokeResult = self.dll.TLPM_setWavelength(
            self.devSession, wavelength)
        self.__testForError(pInvokeResult)
        return pInvokeResult

    def getWavelength(self, attribute, wavelength):
        """
        This function returns the user wavelength in nanometers (nm).

        Wavelength set value is used for calculating power.

        Args:
            attribute(c_int16): This parameter specifies the value to be
                queried.
            ...
            wavelength(c_double use with byref): This parameter returns the
                specified wavelength in nanometers (nm).

        Returns:
            int: The return value, 0 is for success
        """
        pInvokeResult = self.dll.TLPM_getWavelength(
            self.devSession, attribute, wavelength)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def setPowerRange(self, power_to_Measure):
        """
        This function sets the power range of the sensor.

        Args:
            power_to_Measure(c_double): This parameter specifies the most
                positive signal level expected for the sensor input in
                Watts (W).
        ...
        pInvokeResult = self.dll.TLPM_setPowerRange(
            self.devSession, power_to_Measure)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def getPowerRange(self, attribute, powerValue):
        ...
        Args:
            attribute(c_int16): This parameter specifies the value to be
                queried. Acceptable values:
                    TLPM_ATTR_SET_VAL (0): Set value
                    TLPM_ATTR_MIN_VAL (1): Minimum value
                    TLPM_ATTR_MAX_VAL (2): Maximum value

            powerValue(c_double use with byref): This parameter returns the
                specified power range value in Watts (W).

        Returns:
            int: The return value, 0 is for success
        """
        pInvokeResult = self.dll.TLPM_getPowerRange(self.devSession,
                                                    attribute,
                                                    powerValue)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def setPowerUnit(self, powerUnit):
        ...
        Args:
            powerUnit(c_int16): This parameter specifies the unit of the
                power value. Acceptable values:
    ...
    def measPower(self, power):
        ...
        Remark: This function starts a new measurement cycle and after
                finishing measurement the result is received. Subject to the
                actual Average Count this may take up to seconds. Refer to
                <Set/Get Average Count>.

        Args:
            power(c_double use with byref): This parameter returns the power
                in the selected unit. Select the unit with <Set Power Unit>.
    ...
    def errorMessage(self, statusCode, description):
        ...
        Args:
            statusCode(ViStatus): This parameter accepts the error codes
                returned from the instrument driver functions.
            ...
            description(create_string_buffer): This parameter returns the
                ...
                must contain at least 512 elements ViChar[512].

        Returns:
            int: The return value, 0 is for success
        """
        pInvokeResult = self.dll.TLPM_errorMessage(
            self.devSession, statusCode, description)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def selfTest(self, selfTestResult, description):
        ...
        pInvokeResult = self.dll.TLPM_selfTest(
            self.devSession, selfTestResult, description)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def revisionQuery(self, instrumentDriverRevision, firmwareRevision):
        ...
        pInvokeResult = self.dll.TLPM_revisionQuery(
            self.devSession, instrumentDriverRevision, firmwareRevision)
        self.__testForError(pInvokeResult)
        return pInvokeResult

    def identificationQuery(self, manufacturerName, deviceName, serialNumber,
                            firmwareRevision):
        """
        ...
        pInvokeResult = self.dll.TLPM_identificationQuery(
            self.devSession, manufacturerName, deviceName, serialNumber,
            firmwareRevision)
        self.__testForError(pInvokeResult)
        return pInvokeResult
    ...
    def getCalibrationMsg(self, message):
        ...
        Args:
            message(create_string_buffer): This parameter returns the
                calibration message. The array must contain at least
                TLPM_BUFFER_SIZE (256) elements ViChar[256].

        Returns:
            int: The return value, 0 is for success
        """
        pInvokeResult = self.dll.TLPM_getCalibrationMsg(
            self.devSession, message)
        self.__testForError(pInvokeResult)
        return pInvokeResult

    def getSensorInfo(self, name, snr, message, pType, pStype, pFlags):
        """
        This function is used to obtain information from the connected sensor
        like sensor name, serial number, calibration message, sensor type,
        sensor subtype and sensor flags.
        ...
        Args:
            name(create_string_buffer): This parameter returns the name of the
                connected sensor.

            snr(create_string_buffer): This parameter returns the serial
                number of the connected sensor.

            message(create_string_buffer): This parameter returns the
                calibration message of the connected sensor.

            pType(c_int16 use with byref): This parameter returns the sensor
                type of the connected sensor.
                The meanings of the obtained sensor type are:
            ...
            pStype(c_int16 use with byref): This parameter returns the subtype
                ...
                Sensor Subtypes Pyroelectric Sensor:
                 SENSOR_SUBTYPE_PYRO_ADAPTER    0x01 // Pyroelectric adapter
                 SENSOR_SUBTYPE_PYRO_STD        0x02 // Pyroelectric sensor
                 SENSOR_SUBTYPE_PYRO_STD_T      0x12 // Pyroelectric sensor
                                                        with temperature sensor

            pFlags(c_int16 use with byref): This parameter returns the flags
                of the connected sensor.
        ...
        Returns:
            int: The return value, 0 is for success
        """
        pInvokeResult = self.dll.TLPM_getSensorInfo(
            self.devSession, name, snr, message, pType, pStype, pFlags)
        self.__testForError(pInvokeResult)
        return pInvokeResult
