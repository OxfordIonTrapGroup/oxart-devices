...
Waveform = namedtuple("Waveform", "voltage_vec_list,el_vec,fixed_wells,wells_idx")
...
class SURFMediator:
    ...
    def __init__(
        ...
        default_f_axial=1e6,
        default_f_rad_x=5e6,
        default_split_start_curvature=1.0e7,
        default_split_end_curvature=-1.0e7,
        default_split_well_seperation=140e-6,
        default_split_positions=np.array([0.0]),
        charge=1,
        mass=43,
    ):
        """
    ...
    def _volt_from_wells(
        self, wells, electrodes=None, z_grid=None, static_settings=None
    ):
    ...
    def get_model_field(self, zs, wave: Waveform, field="dphidz"):
        ...
        """
        return self.driver.get_model_field(
            zs, wave.voltage_vec_list, wave.el_vec, field
        )
    ...
    def reload_trap_model(
        self, trap_model_path=None, cache_path=None, omega_rf=None, mass=None, v_rf=None
    ):
        ...
        """
        return self.load_trap_model(
            trap_model_path, cache_path, omega_rf, mass, v_rf, force_reload=True
        )
    ...
    def load_trap_model(
        ...
        if mass is not None:
            self.mass = mass
        return self.driver.load_config(
            trap_model_path,
            cache_path,
            omega_rf,
            self.mass,
    ...
    def modify(
        ...
        *,
        ...
        new_volt, new_el = self._volt_from_wells(
            new_wells, electrodes, z_grid, static_settings
        )

        # dict to pass to do_solve
        start_volt_dict = {
            el: wave.voltage_vec_list[-1][idx] for idx, el in enumerate(wave.el_vec)
        }

        end_volt_dict = {el: 0.0 for el in wave.el_vec}
        end_volt_dict.update({el: new_volt[idx] for idx, el in enumerate(new_el)})
    ...
    def split(
        ...
        *,
        ...
        scan_start.rx_axial[0] = 0.0
        scan_start.ry_axial[0] = 0.0
        scan_start.phi_radial[0] = 0.0
        ...
        final_wells = Wells(
            *(
                spectators[i][:split_idx] + split_wells[i] + spectators[i][split_idx:]
    ...
    def merge(
        ...
        *,
        ...
        if prepare_wells:
            ...
            move_dict = {
                name0: {"z": scan_start.z[0] - name_order * well_separation / 2},
                name1: {"z": scan_start.z[0] + name_order * well_separation / 2},
            }
    ...
    def dynamic_split(
        ...
        *,
        ...
        target_well.rx_axial[0] = 0.0
        target_well.ry_axial[0] = 0.0
        target_well.phi_radial[0] = 0.0
        if axial_tilt is not None:
    ...
    def dynamic_merge(
        ...
        *,
        ...
        if prepare_wells:
            ...
            name_order = np.sign(target_well.z[1] - target_well.z[0])
            move_dict = {
                name0: {"z": split_well.z[0] - name_order * well_separation / 2},
                name1: {"z": split_well.z[0] + name_order * well_separation / 2},
            }
            wave = self.modify(move_dict, wave, n_prepare)
    ...
    def spawn_wells(self, z, wave, n_step=5, *, z_grid=None, **kwargs):
        ...
        new_wells = Wells(
            *[[*new_wells[i], *tmp_wells[i]] for i in range(len(new_wells))]
        )

        new_volt, el = self._volt_from_wells(new_wells, wave.el_vec, z_grid)
    ...
    def _poly_interpolate(self, volt0, volt1, n_step):
        ...
        """
        return [
            volt0 + (volt1 - volt0) * (10 * t**3 - 15 * t**4 + 6 * t**5)
